...
__version__ = 'BIDS-App \'MRtrix3_connectome\' version {}'.format(open('/version').read()) \
              if IS_CONTAINER \
...
class ParticipantShared(object): #pylint: disable=useless-object-inheritance
  def __init__(self, atlas_path, output_verbosity, parcellation, preprocessed, streamlines, template_reg):
    ...
    if not parcellation:
      app.error('For participant-level analysis, desired parcellation must be provided using the ' + \
    ...
    eddy_help_process = subprocess.Popen([ self.eddy_binary, '--help' ], \
                                         stdin=None, \
    ...
    if template_reg:
      if self.do_mni:
        ...
        app.warn('Volumetric template registration not being performed; ' + \
    ...
    if self.template_registration_software == 'ants':
      if not find_executable('ANTS') or not find_executable('WarpImageMultiTransform'):
        app.error('Commands \'ANTS\' and \'WarpImageMultiTransform\' must be present in PATH ' + \
                  'to use ANTs software for template registration')
      ...
      if not os.path.isfile(self.fnirt_config_path):
        app.error('Unable to find configuration file for FNI FNIRT ' + \
                  '(expected location: ' + self.fnirt_config_path + ')')
    ...
    if self.do_freesurfer:
      ...
      for subdir in [ 'fsaverage', 'fsaverage5', 'lh.EC_average', 'rh.EC_average' ]:
        if not os.path.isdir(os.path.join(self.freesurfer_subjects_dir, subdir)):
          app.error('Could not find requisite FreeSurfer subject directory \'' + subdir + '\' ' + \
                    '(expected location: ' + os.path.join(self.freesurfer_subjects_dir, subdir) + ')')
      ...
      if parcellation in [ 'hcpmmp1', 'yeo7fs', 'yeo17fs' ]:
        if parcellation == 'hcpmmp1':
          self.hcpmmp1_annot_paths = [ os.path.join(self.freesurfer_subjects_dir, \
                                                    'fsaverage', \
                                                    'label', \
          ...
          self.yeo_annot_paths = [ os.path.join(self.freesurfer_subjects_dir, \
                                                'fsaverage5', \
                                                'label', \
                                                hemi + 'h.Yeo2011_' + \
        ...
        for cmd in [ 'mri_surf2surf', 'mri_aparc2aseg' ]:
          if not find_executable(cmd):
            app.error('Could not find FreeSurfer command ' + cmd + ' ' + \
                      '(necessary for applying HCPMMP1 parcellation); ' + \
                      'please verify FreeSurfer installation')
      elif parcellation == 'brainnetome246fs':
        self.brainnetome_cortex_gcs_paths = [ os.path.join(self.freesurfer_home, \
                                                           'average', \
                                                           hemi + 'h.BN_Atlas.gcs') \
                                              for hemi in [ 'l', 'r' ] ]
        if not all([os.path.isfile(path) for path in self.brainnetome_cortex_gcs_paths]):
          app.error('Could not find necessary GCS files for applying ' + \
                    ...
                    os.path.join(self.freesurfer_home, \
                                 ...
                                 '?h.BN_Atlas.gcs') + \
                    ')')
        self.brainnetome_sgm_gca_path = os.path.join(self.freesurfer_home, \
                                                     'average', \
        ...
        if not os.path.isfile(self.brainnetome_sgm_gca_path):
          app.error('Could not find necessary GCA file for applying ' + \
                    'Brainnetome sub-cortical parcellation via FreeSurfer ' + \
      ...
      try:
        self.freesurfer_template_link_function(self.freesurfer_subjects_dir, \
                                               'test_softlink')
        ...
        app.debug('Unable to create softlinks; ' + \
                  'will perform deep copies of FreeSurfer template directories')
        self.freesurfer_template_link_function = shutil.copytree

    elif self.do_mni:
      self.template_image_path = os.path.join(fsl_path, \
                                              'data', \
                                              'standard', \
                                              'MNI152_T1_2mm.nii.gz')
      self.template_mask_path = os.path.join(fsl_path, \
                                             'data', \
                                             'standard', \
                                             'MNI152_T1_2mm_brain_mask.nii.gz')
      if parcellation == 'aal':
        self.parc_image_path = os.path.abspath(os.path.join(os.sep, \
                                                            'opt', \
                                                            'aal', \
                                                            'ROI_MNI_V4.nii'))
        self.parc_lut_file = os.path.abspath(os.path.join(os.sep, \
                                                          'opt', \
                                                          'aal', \
                                                          'ROI_MNI_V4.txt'))
        self.mrtrix_lut_file = os.path.join(mrtrix_lut_dir, \
    ...
    def findAtlasFile(filepath, description):
      ...
      if os.path.isfile(newpath):
        return newpath
      app.error('Could not find ' + description + ' ' + \
                '(tested locations: \'' + filepath + '\', \'' + newpath + '\')')
      return None
...
def runSubject(bids_dir, label, shared, output_prefix):
  ...
  if os.path.exists(output_dir):
    app.warn('Output directory for subject \'' + label + '\' already exists; ' + \
             'contents will be erased when this execution completes')
  ...
  #   (i.e. not just those used for estimating the inhomogeneity field), they
  #   will need to be stored as separate NIfTI files in the 'dwi/' directory.
  ...
  if os.path.isdir(fmap_dir):
    if shared.preprocessed:
      app.error('fmap/ directory detected for subject \'' + label + '\' despite use of ' + \
                OPTION_PREFIX + 'preprocessed option; ' + \
                'this directory should not be present if DWIs have already been pre-processed')
    ...
    for entry in fmap_image_list:
      ...
      if 'IntendedFor' in json_elements and \
          not any(i.endswith(json_elements['IntendedFor']) for i in dwi_image_list):
  ...
  #   contrast within the input DWI(s)
    ...
    app.error('Inadequate data for pre-processing of subject \'' + label + '\': ' + \
              'No phase-encoding contrast in input DWIs, and no fmap/ directory, ' + \
  ...
  if shared.preprocessed:
    ...
    # Do initial image pre-processing (denoising, Gibbs ringing removal if available,
    #   distortion correction & bias field correction) as normal
    ...
    if fmap_num_volumes:
      ...
      run.command('mrconvert ' + mrdegibbs_output + ' ' + dwipreproc_se_epi + ' ' + \
                  '-coord 3 0:' + str(fmap_num_volumes-1))
      cat_num_volumes = image.Header(mrdegibbs_output).size()[3]
      app.var(cat_num_volumes)
      run.command('mrconvert ' + mrdegibbs_output + ' ' + dwipreproc_input + ' ' + \
                  '-coord 3 ' + str(fmap_num_volumes) + ':' + str(cat_num_volumes-1))
    ...
    dwipreproc_eddy_option = ' -eddy_options \" ' + \
                             ' '.join(eddy_options) + '\"' if eddy_options else ''
    run.function(os.makedirs, 'eddyqc')
    run.command('dwipreproc ' + dwipreproc_input + ' dwi_preprocessed.mif -rpe_header' + \
                dwipreproc_se_epi_option + dwipreproc_eddy_option + ' ' + \
                '-eddyqc_text eddyqc/')
  ...
  run.command('dwi2response dhollander dwi.mif response_wm.txt response_gm.txt response_csf.txt ' + \
              '-mask dwi_mask.mif')
  ...
  T1_revert_strides_option = ' -strides ' + \
                             ','.join([str(i) for i in T1_header.strides()])
  ...
  run.command('mrcalc 1 dwi_meanbzero.mif -div dwi_mask.mif -mult - | ' + \
              'mrhistmatch nonlinear - T1_biascorr_brain.mif dwi_pseudoT1.mif ' + \
              '-mask_input dwi_mask.mif ' + \
              '-mask_target T1_mask.mif')
  run.command('mrcalc 1 T1_biascorr_brain.mif -div T1_mask.mif -mult - | '
              'mrhistmatch nonlinear - dwi_meanbzero.mif T1_pseudobzero.mif ' + \
  ...
  run.command('transformcalc rigid_T1_to_pseudoT1.txt rigid_pseudobzero_to_bzero.txt ' + \
  ...
  # Note: Since we're using a mask from fsl_anat (which crops the FoV), but using it as input to 5ttgen fsl
  ...
  if shared.do_freesurfer:
      ...
      if shared.parcellation == 'brainnetome246fs':
        ...
        run.command('mrcalc ' + \
                    ...
                    os.path.join('freesurfer', \
                                 'mri', \
                                 'BN_Atlas_subcortex.mgz') + ' ' + \
                    '1.0 sgm_overlap.mif -sub -mult -add ' + parc_image_path)
    ...
    app.console('Registering to MNI template and transforming grey matter ' + \
                'parcellation back to subject space')

    # Use non-dilated brain masks for performing histogram matching & linear registration
    run.command('mrhistmatch linear T1_registered.mif ' + shared.template_image_path + ' ' + \
    ...
    if shared.template_registration_software == 'ants':
      ...
      run.command('ANTS 3 ' + \
                  '-m PR[' + shared.template_image_path + ', T1_registered_histmatch.nii, 1, 2] ' + \
                  '-o ANTS ' + \
                  '-r Gauss[2,0] ' + \
                  '-t SyN[0.5] ' + \
                  '-i 30x99x11 ' + \
      ...
      run.command('WarpImageMultiTransform 3 ' + shared.parc_image_path + ' ' + transformed_atlas_path + ' ' + \
      ...
      run.command('mrcalc T1_registered_histmatch.nii T1_mask_registered.mif -mult ' + \
                  'T1_registered_histmatch_masked.nii')
      # Template T1, brain masked; for flirt -ref
      run.command('mrcalc ' + shared.template_image_path + ' ' + shared.template_mask_path + ' -mult ' + \
                  'template_masked.nii')
      ...
      run.command('maskfilter ' + shared.template_mask_path + ' dilate template_mask_dilated.nii ' + \
                  '-npass 3')
    ...
    if shared.parc_lut_file and shared.mrtrix_lut_file:
      run.command('labelconvert ' + transformed_atlas_path + ' ' + \
                                    shared.parc_lut_file + ' ' + \
                                    shared.mrtrix_lut_file + ' ' + \
                                    'parc.mif')
  ...
  if shared.streamlines:
    ...
    # If not manually specified, determine the appropriate number of streamlines
    #   based on the number of nodes in the parcellation:
  ...
  if num_streamlines:
    ...
    while num_streamlines:
      run.command('tcksift2 ' + tractogram_filepath + ' FOD_WM.mif weights.csv ' + \
                  '-act 5TT.mif ' + \
                  '-out_mu mu.txt' + \
      ...
      run.command('tckedit ' + tractogram_filepath + ' ' + new_tractogram_filepath + ' ' + \
    ...
    if shared.output_verbosity > 2:
      ...
      run.command('tckmap ' + tractogram_filepath + ' tdi_highres.mif ' + \
                  '-tck_weights_in weights.csv ' + \
                  '-template vis.mif ' + \
                  '-vox ' + ','.join([str(value/3.0) for value in image.Header('vis.mif').spacing() ]) + ' ' + \
                  '-datatype uint16')
  ...
  if shared.parcellation != 'none':
    ...
    assignment_option = ' -assignment_radial_search 5' \
    ...
    if shared.output_verbosity > 2:
      ...
      run.command('connectome2tck ' + tractogram_filepath + ' assignments.csv exemplars.tck ' + \
                  '-tck_weights_in weights.csv ' + \
                  '-exemplars parc.mif ' + \
                  '-files single')
  ...
  if lut_export_file:
    lut_export_path = os.path.join(output_prefix, \
                                   parc_string[1:] + '_lookup' + os.path.splitext(lut_export_file)[1])
    try:
  ...
  if shared.parcellation != 'none':
    run.function(shutil.copy, \
                 ...
                 os.path.join(output_dir, \
                              'connectome', \
                              label + parc_string + '_level-participant_connectome.csv'))
  if num_streamlines:
    run.function(shutil.copy, \
                 'mu.txt', \
                 os.path.join(output_dir, \
  ...
  if shared.output_verbosity > 2:
    if num_streamlines:
      ...
      run.function(shutil.copy if shared.output_verbosity == 4 else shutil.move, \
                   tractogram_filepath, \
                   os.path.join(output_dir, \
                                'tractogram', \
    ...
    if shared.parcellation != 'none':
      run.function(shutil.copy, \
                   ...
                   os.path.join(output_dir, \
                                'connectome', \
                                label + parc_string + '_assignments.csv'))
      run.function(shutil.copy, \
                   'exemplars.tck', \
                   os.path.join(output_dir, \
                                'connectome', \
...
def runGroup(output_dir):
  ...
  class subjectPaths(object):
    def __init__(self, label):
      ...
      self.in_rf = os.path.join(output_dir, \
                                ...
                                'dwi', \
                                label + '_tissue-WM_response.txt')
      connectome_files = glob.glob(os.path.join(output_dir, \
                                                label, \
                                                'connectome', \
      ...
      if not connectome_files:
        ...
        app.error('Connectomes from multiple parcellations detected for subject \'' + label + '\'; ' + \
      ...
      self.parcellation = re.findall('(?<=_parc-)[a-zA-Z0-9]*', \
      ...
      self.temp_connectome = os.path.join('connectomes', \
                                          label + '.csv')
      self.out_scale_intensity = os.path.join(output_dir, \
                                              label, \
                                              'connectome', \
                                              label + '_factor-intensity_multiplier.txt')
      self.out_scale_RF = os.path.join(output_dir, \
                                       label,
                                       'connectome',
                                       label + '_factor-response_multiplier.txt')
      self.out_connectome = os.path.join(output_dir, \
                                         label, \
                                         'connectome', \
                                         os.path.basename(self.in_connectome).replace('_level-participant', \
                                                                                      '_level-group'))
  ...
  progress = app.progressBar('Generating group-average response function and ' + \
  ...
  progress = app.progressBar('Applying normalisation scaling to subject connectomes', \
                             len(subjects))
  ...
  for s in subjects:
    ...
    # Don't want to be scaling connectome independently for differences in
    #   RF l=0 terms across all shells;
...
app.init('Robert E. Smith (robert.smith@florey.edu.au)',
         'Generate structural connectomes based on diffusion-weighted and T1-weighted image data ' + \
         'using state-of-the-art reconstruction tools, particularly those provided in MRtrix3')
...
if IS_CONTAINER:
  ...
  app.cmdline._action_groups[2].add_argument('-d', '--debug', \
                                             dest='debug', \
                                             action='store_true', \
                                             help='In the event of encountering an issue with the script, ' + \
                                                  're-run with this flag set to provide more useful ' + \
                                                  'information to the developer')
  app.cmdline._action_groups[2].add_argument('-h', '--help', \
                                             dest='help', \
                                             action='store_true', \
                                             help='Display help information for the script')
  app.cmdline._action_groups[2].add_argument('-n', '--n_cpus', \
                                             type=int, \
                                             metavar='number', \
                                             dest='nthreads', \
                                             help='Use this number of threads in MRtrix3 multi-threaded applications ' + \
...
app.cmdline.add_argument('output_dir', \
                         help='The directory where the output files should be stored. ' + \
                              'If you are running group level analysis, this folder should be ' + \
...
batch_options.add_argument(OPTION_PREFIX + 'participant_label', \
                           nargs='+', \
                           help='The label(s) of the participant(s) that should be analyzed. ' + \
                                'The label(s) correspond(s) to sub-<participant_label> from the BIDS spec ' + \
                                '(so it does _not_ include "sub-"). ' + \
...
if not IS_CONTAINER:
  participant_options.add_argument(OPTION_PREFIX + 'atlas_path', \
                                   metavar='path', \
                                   help='The filesystem path in which to search for an atlas parcellation ' + \
                                        '(may be necessary if not installed in the same location as they are ' + \
                                        'placed within the MRtrix3_connectome BIDS App container)')
participant_options.add_argument(OPTION_PREFIX + 'output_verbosity', \
                                 type=int, \
                                 default=2, \
                                 help='The verbosity of script output (number from 1 to 4); ' + \
                                      'higher values result in more generated data being included ' + \
                                      'in the output directory (default = 2)')
...
participant_options.add_argument(OPTION_PREFIX + 'preprocessed', \
                                 action='store_true', \
                                 help='Indicate that the subject DWI data have been preprocessed, and hence ' + \
                                      'initial image processing steps will be skipped')
participant_options.add_argument(OPTION_PREFIX + 'streamlines', \
                                 type=int, \
                                 default=0, \
...
if not IS_CONTAINER:
  app.cmdline.addCitation('If ' + OPTION_PREFIX + 'preprocessed is not used', \
                          ...
                          True)
app.cmdline.addCitation('If ' + OPTION_PREFIX + 'preprocessed is not used', \
                        'Andersson, J. L. R.; Graham,, M. S.; Drobnjak, I.; Zhang, H. & Campbell, J. ' + \
                        ...
                        True)
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation [ aal, aal2, brainnetome246mni, craddock200, craddock400, perry512, yeo7mni or yeo17mni ], and not using ' + OPTION_PREFIX + 'template_reg fsl', \
...
app.cmdline.addCitation('', \
                        ...
                        True)
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation [ craddock200 or craddock400 ]', \
                        ...
                        True)
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation [ brainnetome246fs, desikan, destrieux, hcpmmp1, yeo7fs or yeo17fs ]', \
...
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation desikan', \
                        'Desikan, R. S.; Segonne, F.; Fischl, B.; Quinn, B. T.; Dickerson, B. C.; Blacker, D.; Buckner, R. L.; Dale, A. M.; Maguire, R. P.; Hyman, B. T.; Albert, M. S. & Killiany, R. J. ' + \
...
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation destrieux', \
                        ...
                        True)
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation [ brainnetome246fs brainnetome246mni ]', \
...
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation hcpmmp1', \
                        'Glasser, M. F.; Coalson, T. S.; Robinson, E. C.; Hacker, C. D.; Harwell, J.; Yacoub, E.; Ugurbil, K.; Andersson, J.; Beckmann, C. F.; Jenkinson, M.; Smith, S. M. & Van Essen, D. C. ' + \
                        'A multi-modal parcellation of human cerebral cortex. ' + \
                        'Nature, 2016, 536, 171-178', \
                        True)
app.cmdline.addCitation('' if IS_CONTAINER else 'If using ROBEX for brain extraction', \
...
app.cmdline.addCitation('If ' + OPTION_PREFIX + 'preprocessed is not used', \
                        ...
                        True)
app.cmdline.addCitation('If not using ' + OPTION_PREFIX + 'parcellation [ brainnetome246fs or brainnetome246mni ]', \
...
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation perry512', \
                        'Perry, A.; Wen, W.; Kochan, N. A.; Thalamuthu, A.; Sachdev, P. S.; Breakspear, M. ' + \
...
app.cmdline.addCitation('', \
                        ...
                        False)
app.cmdline.addCitation('If ' + OPTION_PREFIX + 'preprocessed is not used', \
                        'Tustison, N.; Avants, B.; Cook, P.; Zheng, Y.; Egan, A.; Yushkevich, P. & Gee, J. ' + \
                        'N4ITK: Improved N3 Bias Correction. ' + \
                        'IEEE Transactions on Medical Imaging, 2010, 29, 1310-1320', \
                        True)
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation [ aal or aal2 ]', \
...
app.cmdline.addCitation('', \
                        ...
                        False)
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation [ yeo7fs, yeo7mni, yeo17fs or yeo17mni ]', \
                        'Yeo, B.T.; Krienen, F.M.; Sepulcre, J.; Sabuncu, M.R.; Lashkari, D.; Hollinshead, M.; Roffman, J.L.; Smoller, J.W.; Zollei, L.; Polimeni, J.R.; Fischl, B.; Liu, H. & Buckner, R.L. ' + \
                        'The organization of the human cerebral cortex estimated by intrinsic functional connectivity. ' + \
                        'J Neurophysiol, 2011, 106(3), 1125-1165', \
                        False)
app.cmdline.addCitation('If using ' + OPTION_PREFIX + 'parcellation perry512', \
                        'Zalesky, A.; Fornito, A.; Harding, I. H.; Cocchi, L.; Yucel, M.; Pantelis, C. & Bullmore, E. T. ' + \
