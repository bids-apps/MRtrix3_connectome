...
__version__ = 'BIDS-App \'MRtrix3_connectome\' version {}'.format(
    open(
        '/version').read()) if is_container else 'BIDS-App \'MRtrix3_connectome\' standalone'
option_prefix = '--' if is_container else '-'


def runSubject(bids_dir, label, output_prefix, session=None):
    if session is not None:
        output_dir = os.path.join(output_prefix, label, session)
    else:
        output_dir = os.path.join(output_prefix, label)

    if os.path.exists(output_dir):
        app.warn('Output directory for subject \'' + label +
                 '\' already exists; contents will be erased when this execution completes')
    ...
    if robex_found and N4_found:
        app.console(
            'N4BiasFieldCorrection and ROBEX found; will use for bias field correction and brain extraction')
        brain_extraction_cmd = 'runROBEX.sh'
    else:
        if robex_found and not N4_found:
            app.console(
                'N4BiasFieldCorrection not found; will use FSL for bias field correction & brain extraction')
        elif N4_found and not robex_found:
            app.console('ROBEX not found; will use FSL for brain extraction')
        else:
            app.console(
                'N4BiasFieldCorrection and ROBEX not found; will use FSL for brain extraction')
    ...
    if not app.args.parcellation:
        app.error(
            'For participant-level analysis, desired parcellation must be provided using the ' + option_prefix +
            'parcellation option')

    if app.args.template_reg:
        if app.args.parcellation in ['aal', 'aal2', 'craddock200',
                                     'craddock400', 'perry512']:
            template_registration_software = app.args.template_reg
        else:
            app.warn(
                'Template registration not being performed; ' + option_prefix + 'template_reg option ignored')
            template_registration_software = ''
    else:
        template_registration_software = 'ants' if app.args.parcellation in [
            'aal', 'aal2', 'craddock200',
            'craddock400', 'perry512'] else ''

    if template_registration_software == 'ants':
        if not find_executable('ANTS') or not find_executable(
                'WarpImageMultiTransform'):
            app.error(
                'Commands \'ANTS\' and \'WarpImageMultiTransform\' must be present in PATH to use ANTs software for '
        ...
        fnirt_config_path = os.path.join(fsl_path, 'etc', 'flirtsch',
                                         fnirt_config_basename)
        if not os.path.isfile(fnirt_config_path):
            app.error(
                'Unable to find configuration file for FNI FNIRT (expected location: ' + fnirt_config_path + ')')
    ...
    if app.args.parcellation in ['desikan', 'destrieux', 'hcpmmp1']:
        ...
        freesurfer_subjects_dir = os.environ['SUBJECTS_DIR'] if 'SUBJECTS_DIR' in os.environ else os.path.join(
            os.environ['FREESURFER_HOME'], 'subjects')

        if not os.path.isdir(freesurfer_subjects_dir):
            app.error(
                'Could not find FreeSurfer subjects directory (expected location: ' + freesurfer_subjects_dir + ')')

        for subdir in ['fsaverage', 'lh.EC_average', 'rh.EC_average']:
            if not os.path.isdir(os.path.join(freesurfer_subjects_dir, subdir)):
                app.error(
                    'Could not find requisite FreeSurfer subject directory \'' + subdir + '\' (expected location: ' +
                    os.path.join(
                        freesurfer_subjects_dir, subdir) + ')')

        if app.args.parcellation == 'hcpmmp1':
            if not all([os.path.isfile(
                    os.path.join(freesurfer_subjects_dir, 'fsaverage', 'label',
                                 hemi + 'h.HCPMMP1.annot')) for hemi in ['l', 'r']]):
                app.error('Could not find necessary annotation labels for applying HCPMMP1 parcellation '
                          '(expected location: ' + os.path.join(freesurfer_subjects_dir, 'fsaverage',
                                                                'label', '?h.HCPMMP1.annot'))
        ...
        if app.args.parcellation == 'hcpmmp1':
            for cmd in ['mri_surf2surf', 'mri_aparc2aseg']:
                if not find_executable(cmd):
                    app.error('Could not find FreeSurfer command ' +
                              cmd + ' (necessary for applying HCPMMP1 parcellation); '
            ...
            if not all(os.path.isfile(
                    os.path.join(freesurfer_subjects_dir, 'fsaverage', 'label',
                                 hemi + 'h.HCPMMP1.annot')) for hemi in ['l', 'r']):
                app.error(
                    'Could not find FreeSurfer annotation files for HCPMMP1 parcellation (expected location: ' +
                    os.path.join(freesurfer_subjects_dir, 'fsaverage', 'label', '?h.HCPMMP1.annot'))
        ...
        if app.numThreads is None or app.numThreads:
            ...
            for line in reconall_text:
                ...
                #   allow recon-all to use its own default number of threads
                    ...
                    reconall_multithread_options.extend(
    ...
    if session is not None:
        dwi_image_list = glob.glob(os.path.join(bids_dir, label, session, 'dwi', label + '*_dwi.nii*'))
        dwi_index = 1
    else:
    ...
    for entry in dwi_image_list:
        ...
        if os.path.isfile(json_path):
            ...
            app.error('No sidecar JSON file found for image \'' + entry +
                      '\'; cannot proceed with DWI preprocessing without this information')
    ...
    if session is not None:
        fmap_dir = os.path.join(bids_dir, label, session, 'fmap')
        fmap_index = 1
    else:
    ...
    if os.path.isdir(fmap_dir):
        if app.args.preprocessed:
            app.error('fmap/ directory detected for subject \'' + label + '\' despite use of ' + option_prefix +
                      'preprocessed option; this directory should not be present if DWIs have already been pre-processed')
        ...
        for entry in fmap_image_list:
            ...
            if 'IntendedFor' in json_elements and not any(
                    i.endswith(json_elements['IntendedFor']) for i in dwi_image_list):
            ...
            run.command('mrconvert ' + entry + ' -json_import ' + json_path + ' ' + '-grad ' +
                        path.toTemp(fmap_dwscheme_file, True) + ' ' +
    ...
    #   contrast within the input DWI(s)
        ...
        app.error('Inadequate data for pre-processing of subject \'' + label + session +
                  '\': No phase-encoding contrast in input DWIs, and no fmap/ directory, so EPI distortion correction cannot be performed')
    ...
    if app.args.fmriprep:
        t1w_image_list = glob.glob(os.path.join('fmriprep/fmriprep', label, 'anat', label + '_desc-preproc_T1w.nii.gz'))

        if len(t1w_image_list) > 1:
            app.error('More than one T1-weighted image found for subject ' + label + '; check fMRIPrep output')

        run.command('mrconvert ' + t1w_image_list[0] + ' ' + path.toTemp('T1.mif', True))
    else:
        if session is not None:
            t1w_image_list = glob.glob(os.path.join(bids_dir, label, session, 'anat', label + '*_T1w.nii*'))
        else:
            t1w_image_list = glob.glob(os.path.join(bids_dir, label, 'anat', label + '*_T1w.nii*'))

        if len(t1w_image_list) > 1:
            app.error('More than one T1-weighted image found for subject ' + label +
    ...
    if app.args.preprocessed:
        ...
        app.console(
            'Skipping MP-PCA denoising, Gibbs ringing removal, distortion correction and bias field correction due to use of ' +
            option_prefix + 'preprocessed option')

        run.function(os.rename, dwi_image_list[0], 'dwi.mif')

    else:  # Do initial image pre-processing (denoising, Gibbs ringing removal if available, distortion correction &
        ...
        if fmap_image_list:
            ...
            if len(fmap_image_list) > 1:
                ...
                fmap_num_volumes = 1 if len(fmap_header.size()) < 4 else \
                    fmap_header.size()[3]
                run.command('mrcat ' + fmap_image_list[0] + ' ' + ' '.join(dwi_image_list) + ' ' +
        ...
        if fmap_num_volumes:
            ...
            run.command(
                'mrconvert ' + mrdegibbs_output + ' ' + dwipreproc_se_epi + ' -coord 3 0:' + str(fmap_num_volumes - 1))
            cat_num_volumes = image.Header(mrdegibbs_output).size()[3]
            app.var(cat_num_volumes)
            run.command('mrconvert ' + mrdegibbs_output + ' ' + dwipreproc_input + ' -coord 3 ' + str(
                fmap_num_volumes) + ':' + str(cat_num_volumes - 1))
        ...
        if 'SliceDirection' in dwipreproc_input_header.keyval():
            ...
            if 'i' in slice_direction_code:
                ...
                num_slices = dwipreproc_input_header.size()[2]
                app.warn(
                    'Error reading BIDS field \'SliceDirection\' (value: \'' + slice_direction_code + '\'); assuming third axis')
        else:
            num_slices = dwipreproc_input_header.size()[2]
        ...
        eddy_help_process = subprocess.Popen([eddy_binary, '--help'],
        ...
        run.command(
            'dwipreproc ' + dwipreproc_input + ' dwi_preprocessed.mif -rpe_header' + dwipreproc_se_epi_option + dwipreproc_eddy_option + ' -eddyqc_text eddyqc/')
    ...
    app.console('Estimating' + ('' if multishell else ' white matter') +
                ' Fibre Orientation Distribution' + ('s' if multishell else ''))
    if multishell:
        run.command('dwi2fod msmt_csd dwi.mif response_wm.txt FOD_WM.mif response_gm.txt FOD_GM.mif response_csf.txt '
                    'FOD_CSF.mif -mask dwi_mask_dilated.mif -lmax 10,0,0')
    ...
    # The interpreted fibre density should then be based on the maximum-b l=0 WM RF coefficient, taking into account
    # mtnormalise factor
    ...
    if not app.args.fmriprep:
        ...
        if brain_extraction_cmd == 'runROBEX.sh':
            ...
            file.delTemporary('T1.nii')
            run.command('mrconvert ' + fsl.findImage('T1.anat' + os.sep + 'T1_biascorr_brain') +
                        ' T1_biascorr_brain.mif' + T1_revert_strides_option)
            run.command('mrconvert ' + fsl.findImage('T1.anat' + os.sep + 'T1_biascorr_brain_mask') +
                        ' T1_mask.mif -datatype bit' + T1_revert_strides_option)
        ...
        run.command('mrconvert T1.mif T1.nii -strides +1,+2,+3')
        run.command(brain_extraction_cmd + ' T1.nii T1_biascorr_brain.nii T1_biascorr_brain_mask.nii')
        file.delTemporary('T1.nii')
    ...
    run.command('mrcalc 1 dwi_meanbzero.mif -div dwi_mask.mif -mult - | '
                'mrhistmatch nonlinear - T1_biascorr_brain.mif dwi_pseudoT1.mif -mask_input dwi_mask.mif -mask_target '
                'T1_mask.mif')
    run.command('mrcalc 1 T1_biascorr_brain.mif -div T1_mask.mif -mult - | '
                'mrhistmatch nonlinear - dwi_meanbzero.mif T1_pseudobzero.mif -mask_input T1_mask.mif -mask_target '
                'dwi_mask.mif')
    ...
    run.command('mrregister T1_biascorr_brain.mif dwi_pseudoT1.mif -type rigid -mask1 T1_mask.mif -mask2 dwi_mask.mif '
                '-rigid rigid_T1_to_pseudoT1.txt')
    file.delTemporary('T1_biascorr_brain.mif')

    run.command('mrregister T1_pseudobzero.mif dwi_meanbzero.mif -type rigid -mask1 T1_mask.mif -mask2 dwi_mask.mif '
    ...
    run.command(
        'mrtransform T1_mask.mif T1_mask_registered.mif -linear rigid_T1_to_dwi.txt -template T1_registered.mif '
    ...
    if app.args.parcellation in ['desikan', 'destrieux', 'hcpmmp1']:
        ...
        try:
            freesurfer_template_link_function(freesurfer_subjects_dir,
                                              'test_softlink')
        ...
        for subdir in ['fsaverage', 'lh.EC_average', 'rh.EC_average']:
            run.function(freesurfer_template_link_function,
                         os.path.join(freesurfer_subjects_dir, subdir), subdir)
        ...
        if app.args.parcellation == 'desikan':
            ...
            for hemi in ['l', 'r']:
                run.command('mri_surf2surf --srcsubject fsaverage --trgsubject freesurfer --hemi ' + hemi +
                            'h --sval-annot ' + os.path.join(freesurfer_subjects_dir, 'fsaverage', 'label',
        ...
        run.command('mrhistmatch linear T1_registered.mif -mask_input T1_mask_registered.mif ' +
                    template_image_path + ' -mask_target ' + template_mask_path +
        ...
        if template_registration_software == 'ants':
            ...
            # Warp: ANTS 3 -m PR[htargeti.nii, hsourcei.nii, 1, 2] -o houtput transformi.nii -r Gauss[2,
            # 0] -t SyN[0.5] -i 30x99x11 -use-Histogram-Matching
            # Reslice: WarpImageMultiTransform 3 hlabeled sourcei.nii houtput labelsi.nii -R htargeti.nii
            # htransformiWarp.nii htransformiAffine.txt --use-NN
            run.command('ANTS 3 -m PR[' + template_image_path + ', T1_registered_histmatch.nii, 1, 2] '
                                                                '-o ANTS -r Gauss[2,0] -t SyN[0.5] -i 30x99x11 --use-Histogram-Matching')
            transformed_atlas_path = 'atlas_transformed.nii'
            run.command('WarpImageMultiTransform 3 ' + parc_image_path + ' ' + transformed_atlas_path +
                        ' -R T1_registered_histmatch.nii -i ANTSAffine.txt ANTSInverseWarp.nii --use-NN')
            ...
            run.command(
                'mrcalc T1_registered_histmatch.nii T1_mask_registered.mif -mult T1_registered_histmatch_masked.nii')
            ...
            run.command(flirt_cmd + ' -ref template_masked.nii -in T1_registered_histmatch_masked.nii -omat '
                                    'T1_to_template.mat -dof 12 -cost leastsq')
            ...
            run.command(fnirt_cmd + ' --config=' + fnirt_config_basename + ' --ref=' +
                        template_image_path + ' --in=T1_registered_histmatch.nii '
                                              '--aff=T1_to_template.mat --refmask=template_mask_dilated.nii '
                                              '--inmask=T1_mask_registered_dilated.nii --cout=T1_to_template_warpcoef.nii')
            ...
            run.command(invwarp_cmd + ' --ref=T1_registered.nii --warp=' +
                        fnirt_warp_subject2template_path + ' --out=template_to_T1_warpcoef.nii')
            file.delTemporary(fnirt_warp_subject2template_path)
            fnirt_warp_template2subject_path = fsl.findImage('template_to_T1_warpcoef')
            run.command(applywarp_cmd + ' --ref=T1_registered.nii --in=' + parc_image_path + ' --warp=' +
                        fnirt_warp_template2subject_path + ' --out=atlas_transformed.nii --interp=nn')
        ...
        if parc_lut_file or mrtrix_lut_file:
            ...
            run.command('labelconvert ' + transformed_atlas_path + ' ' + parc_lut_file + ' ' +
                        mrtrix_lut_file + ' parc.mif')
        else:  # Not all parcellations need to go through the labelconvert step; they may already be numbered
    ...
    if app.args.streamlines:
        ...
        # If not manually specified, determine the appropriate number of streamlines based on the number of nodes in
        # the parcellation:
    ...
    if num_streamlines:
        ...
        run.command('tckgen FOD_WM.mif ' + tractogram_filepath +
                    ' -act 5TT.mif -backtrack -crop_at_gmwmi -maxlength 250 -power 0.33 '
        ...
        while num_streamlines:
            run.command('tcksift2 ' + tractogram_filepath + ' FOD_WM.mif weights.csv -act 5TT.mif -out_mu mu.txt' +
            ...
            run.command('tckedit ' + tractogram_filepath + ' ' + new_tractogram_filepath + ' -number '
                        + str(num_streamlines))
        ...
        if app.args.output_verbosity > 2:
            ...
            run.command('tckmap ' + tractogram_filepath +
                        ' -tck_weights_in weights.csv -template FOD_WM.mif -precise - | mrcalc - ' +
                        str(mu) + ' -mult tdi_native.mif')
            # - Conventional TDI at super-resolution (mostly just because we can)
            run.command('tckmap ' + tractogram_filepath + ' -tck_weights_in weights.csv -template vis.mif -vox '
                        + ','.join([str(value / 3.0) for value in image.Header('vis.mif').spacing()]) +
                        ' -datatype uint16 tdi_highres.mif')
    ...
    if app.args.parcellation != 'none':
        ...
        run.command('tck2connectome ' + tractogram_filepath +
                    ' parc.mif connectome.csv -tck_weights_in weights.csv -out_assignments assignments.csv')
        run.command('tck2connectome ' + tractogram_filepath +
                    ' parc.mif meanlength.csv -tck_weights_in weights.csv -scale_length -stat_edge mean')
        ...
        if app.args.output_verbosity > 2:
            ...
            run.command('connectome2tck ' + tractogram_filepath +
                        ' assignments.csv exemplars.tck -tck_weights_in weights.csv -exemplars parc.mif -files single')
    ...
    app.console(
        'Processing for subject \'' + label + '\' completed; writing results to output directory')
    if os.path.exists(output_dir):
    ...
    if lut_export_file:
        lut_export_path = os.path.join(output_prefix, parc_string[1:] + '_lookup' +
    ...
    if session is not None:
        subjlabel = label + '_' + session
    else:
        subjlabel = label
    if app.args.parcellation != 'none':
        run.function(shutil.copy, 'connectome.csv',
                     os.path.join(output_dir, 'connectome',
                                  subjlabel + parc_string + '_level-participant_connectome.csv'))
    if num_streamlines:
        run.function(shutil.copy, 'mu.txt',
                     os.path.join(output_dir, 'tractogram', subjlabel + '_mu.txt'))
    run.command('mrconvert dwi_meanbzero.mif ' +
                os.path.join(output_dir, 'dwi', subjlabel + '_meanbzero.nii.gz') + ' -strides +1,+2,+3')
    run.command(
        'mrconvert fa.mif ' + os.path.join(output_dir, 'dwi',
                                           subjlabel + '_model-tensor_fa.nii.gz') + ' -strides +1,+2,+3')
    run.function(shutil.copy, 'response_wm.txt', os.path.join(output_dir, 'dwi', subjlabel + '_tissue-WM_response.txt'))

    with open(os.path.join(output_dir, 'dwi', subjlabel + '_bvalues.txt'), 'w') as f:
        f.write(' '.join([str(value) for value in bvalues]))
    if app.args.output_verbosity > 1:
        if app.args.parcellation != 'none':
            run.command('mrconvert parc.mif ' +
                        os.path.join(output_dir, 'anat', label + parc_string + '_indices.nii.gz') +
                        ' -strides +1,+2,+3')
            run.function(shutil.copy, 'meanlength.csv',
                         os.path.join(output_dir, 'connectome', subjlabel + parc_string + '_meanlength.csv'))

        run.command('mrconvert dwi.mif ' + os.path.join(output_dir, 'dwi', subjlabel + '_dwi.nii.gz') +
                    ' -export_grad_fsl ' +
                    os.path.join(output_dir, 'dwi', subjlabel + '_dwi.bvec') + ' ' +
                    os.path.join(output_dir, 'dwi', subjlabel + '_dwi.bval') +
                    ' -strides +1,+2,+3,+4')

        run.command('mrconvert dwi_mask.mif ' +
                    os.path.join(output_dir, 'dwi', subjlabel + '_brainmask.nii.gz') +
                    ' -datatype uint8 -strides +1,+2,+3')
        run.command('mrconvert T1_registered.mif ' +
                    os.path.join(output_dir, 'anat', label + '_T1w.nii.gz') + ' -strides +1,+2,+3')

        run.command('mrconvert T1_mask_registered.mif ' +
                    os.path.join(output_dir, 'anat', label + '_brainmask.nii.gz') +
                    ' -datatype uint8 -strides +1,+2,+3')

        run.command('mrconvert 5TT.mif ' +
                    os.path.join(output_dir, 'anat', label + '_5TT.nii.gz') + ' -strides +1,+2,+3,+4')

        run.command('mrconvert vis.mif ' +
                    os.path.join(output_dir, 'anat', label + '_tissues3D.nii.gz') + ' -strides +1,+2,+3')

        run.command('mrconvert FOD_WM.mif ' +
                    os.path.join(output_dir, 'dwi', subjlabel + '_tissue-WM_ODF.nii.gz') + ' -strides +1,+2,+3,+4')
        if multishell:
            run.function(shutil.copy, 'response_gm.txt',
                         os.path.join(output_dir, 'dwi', subjlabel + '_tissue-GM_response.txt'))

            run.function(shutil.copy, 'response_csf.txt',
                         os.path.join(output_dir, 'dwi', subjlabel + '_tissue-CSF_response.txt'))

            run.command('mrconvert FOD_GM.mif ' +
                        os.path.join(output_dir, 'dwi', subjlabel + '_tissue-GM_ODF.nii.gz') + ' -strides +1,+2,+3,+4')
            run.command(
                'mrconvert FOD_CSF.mif ' + os.path.join(output_dir, 'dwi',
                                                        subjlabel + '_tissue-CSF_ODF.nii.gz') + ' -strides +1,+2,+3,+4')
            run.command('mrconvert tissues.mif ' +
                        os.path.join(output_dir, 'dwi', subjlabel + '_tissue-all.nii.gz') + ' -strides +1,+2,+3,+4')
    ...
    if app.args.output_verbosity > 2:
        if num_streamlines:
            ...
            run.function(shutil.move, tractogram_filepath,
                         os.path.join(output_dir, 'tractogram', subjlabel + '_tractogram.tck'))

            run.function(shutil.copy, 'weights.csv',
                         os.path.join(output_dir, 'tractogram', subjlabel + '_weights.csv'))

            run.command('mrconvert tdi_native.mif ' +
                        os.path.join(output_dir, 'tractogram', subjlabel + '_variant-native_tdi.nii.gz') +
                        ' -strides +1,+2,+3')

            run.command('mrconvert tdi_highres.mif ' +
                        os.path.join(output_dir, 'tractogram', subjlabel + '_variant-highres_tdi.nii.gz') +
                        ' -strides +1,+2,+3')

        if app.args.parcellation != 'none':
            run.function(shutil.copy, 'assignments.csv',
                         os.path.join(output_dir, 'connectome', subjlabel + parc_string + '_assignments.csv'))

            run.function(shutil.copy, 'exemplars.tck',
                         os.path.join(output_dir, 'connectome', subjlabel + parc_string + '_exemplars.tck'))

            run.function(shutil.copy, 'nodes_smooth.obj',
                         os.path.join(output_dir, 'anat', label + parc_string + '.obj'))

            run.command('mrconvert parcRGB.mif ' +
                        os.path.join(output_dir, 'anat', label + parc_string + '_colour.nii.gz') + ' -strides +1,+2,+3')
    ...
    if app.cleanup:
        ...
        # Can't use run.function() here; it'll try to write to the log file that resides in the temp directory just
        # deleted
        shutil.rmtree(app.tempDir)
    else:
        app.console(
            'Contents of temporary directory kept; location: ' + app.tempDir)
    app.tempDir = ''
...
def runGroup(output_dir):
    ...
    class subjectPaths(object):
        def __init__(self, label):
            self.in_bzero = os.path.join(output_dir, label, 'dwi',
                                         label + '_meanbzero.nii.gz')
            self.in_fa = os.path.join(output_dir, label, 'dwi',
                                      label + '_model-tensor_fa.nii.gz')
            self.in_rf = os.path.join(output_dir, label, 'dwi',
                                      label + '_tissue-WM_response.txt')
            connectome_files = glob.glob(os.path.join(output_dir, label, 'connectome',
                                                      label + '_parc-*_level-participant_connectome.csv'))
            if not connectome_files:
                app.error('No participant-level connectome file found for subject \'' + label + '\'')
            elif len(connectome_files) > 1:
                app.error('Connectomes from multiple parcellations detected for subject \'' +
                          label + '\'; this is not yet supported')
            ...
            self.out_scale_intensity = os.path.join(output_dir, label,
                                                    'connectome', label + '_factor-intensity_multiplier.txt')
            self.out_scale_RF = os.path.join(output_dir, label, 'connectome', label + '_factor-response_multiplier.txt')
            self.out_connectome = os.path.join(output_dir, label, 'connectome',
                                               os.path.basename(self.in_connectome).replace(
                                                   '_level-participant', '_level-group'))
    ...
    progress = app.progressBar('Generating group-average response function and intensity normalisation factors',
                               len(subjects) + 1)
    run.function(os.makedirs, 'voxels')
    sum_median_bzero = 0.0
    sum_RF = []
    for s in subjects:
        run.command('mrtransform template.mif -warp_full ' + s.temp_warp + ' - -from 2 -template ' +
                    s.link_bzero + ' | mrthreshold - ' + s.temp_voxels + ' -abs 0.4')
...
parcellation_choices = ['aal', 'aal2', 'craddock200', 'craddock400', 'desikan',
                        'destrieux', 'hcpmmp1', 'none', 'perry512']
registration_choices = ['ants', 'fsl']

app.init('Robert E. Smith (robert.smith@florey.edu.au)',
         'Generate structural connectomes based on diffusion-weighted and T1-weighted image data using '
         'state-of-the-art reconstruction tools, particularly those provided in MRtrix3')
...
if is_container:
    ...
    app.cmdline._action_groups[2].add_argument('-d', '--debug', dest='debug',
                                               action='store_true',
                                               help='In the event of encountering an issue with the script, '
                                                    're-run with this flag set to provide more useful information to '
                                                    'the developer')

    app.cmdline._action_groups[2].add_argument('-h', '--help', dest='help',
                                               action='store_true',
                                               help='Display help information for the script')

    app.cmdline._action_groups[2].add_argument('-n', '--n_cpus', type=int,
                                               metavar='number',
                                               dest='nthreads',
                                               help='Use this number of threads in MRtrix3 multi-threaded '
                                                    'applications (0 disables multi-threading)')

    app.cmdline._action_groups[2].add_argument('-s', '--skip-bids-validator',
                                               dest='skipbidsvalidator',
                                               action='store_true',
                                               help='Skip BIDS validation')

    app.cmdline._action_groups[2].add_argument('-v', '--version',
                                               action='version',
                                               version=__version__)
else:
    app.cmdline._action_groups[2].add_argument(option_prefix + 'skip-bids-validator',
                                               dest='skipbidsvalidator',
                                               action='store_true', help='Skip BIDS validation')

app.cmdline.add_argument('bids_dir',
                         help='The directory with the input dataset formatted according to the BIDS standard.')

app.cmdline.add_argument('output_dir',
                         help='The directory where the output files should be stored. If you are running group level '
                              'analysis, this folder should be prepopulated with the results of the participant level '
                              'analysis.')

app.cmdline.add_argument('analysis_level',
                         help='Level of the analysis that will be performed. Multiple participant level analyses can '
                              'be run independently (in parallel) using the same output_dir. '
                              'Options are: ' + ', '.join(analysis_choices), choices=analysis_choices)

batch_options = app.cmdline.add_argument_group('Options specific to the batch processing of subject data')

batch_options.add_argument(option_prefix + 'participant_label', nargs='+',
                           help='The label(s) of the participant(s) that should be analyzed. The label(s) correspond('
                                's) to sub-<participant_label> from the BIDS spec (so it does _not_ include "sub-"). '
                                'If this parameter is not provided, all subjects will be analyzed sequentially. '
                                'Multiple participants can be specified with a space-separated list.')

batch_options.add_argument(option_prefix + 'session_label', nargs='+',
                           help='The label(s) of the session(s) that should be analyzed. The label(s) correspond('
                                's) to ses-<session_label> from the BIDS spec (so it does _not_ include "ses-"). '
                                'If this parameter is not provided, all sessions will be analyzed sequentially. '
...
if not is_container:
    participant_options.add_argument(option_prefix + 'atlas_path',
                                     metavar='path',
                                     help='The filesystem path in which to search for an atlas parcellation (may be '
                                          'necessary if not installed in the same location as they are placed within '
                                          'the BIDS App container)')

participant_options.add_argument(option_prefix + 'output_verbosity',
                                 type=int,
                                 default=2,
                                 help='The verbosity of script output (number from 1 to 3); higher values result in '
                                      'more generated data being included in the output directory')

# TODO Would like to modify script to handle more than one parcellation within a single execution
#  - Would need to restructure subject-level analysis to perform processing steps needed for multiple parcellations
#  only once
...
participant_options.add_argument(option_prefix + 'parcellation',
                                 help='The choice of connectome parcellation scheme (compulsory for participant-level '
                                      'analysis). Options are: ' + ', '.join(parcellation_choices),
                                 choices=parcellation_choices)

participant_options.add_argument(option_prefix + 'preprocessed',
                                 action='store_true',
                                 help='Indicate that the subject DWI data have been preprocessed, and hence initial '
                                      'image processing steps will be skipped')

participant_options.add_argument(option_prefix + 'streamlines', type=int,
                                 help='The number of streamlines to generate for each subject (will be determined '
                                      'heuristically if not explicitly set)')

participant_options.add_argument(option_prefix + 'template_reg',
                                 metavar='software',
                                 help='The choice of registration software for mapping subject to template space. '
                                      'Options are: ' + ', '.join(registration_choices),
                                 choices=registration_choices)

participant_options.add_argument(option_prefix + 'fmriprep',
                                 dest="fmriprep",
                                 action='store_true',
                                 help='Indicate that fMRIPrep has been run and so preprocessed T1s be used.')

participant_options.add_argument(option_prefix + 'longitudinal',
                                 dest="longitudinal",
                                 action="store_true",
                                 help='The label(s) of the participant(s) that should be analyzed. The label(s) correspond('
                                      's) to sub-<participant_label> from the BIDS spec (so it does _not_ include "sub-"). '
...
app.cmdline.addCitation('If ' + option_prefix + 'preprocessed is not used',
                        'Andersson, J. L.; Skare, S. & Ashburner, J. How to correct susceptibility distortions in '
                        'spin-echo echo-planar images: application to diffusion tensor imaging. NeuroImage, 2003, 20, '
                        '870-888', True)
app.cmdline.addCitation('If using ' + option_prefix + 'template_reg fsl',
                        'Andersson, J. L. R.; Jenkinson, M. & Smith, S. Non-linear registration, aka spatial '
                        'normalisation. FMRIB technical report, 2010, TR07JA2', True)
app.cmdline.addCitation('If ' + option_prefix + 'preprocessed is not used',
                        'Andersson, J. L. & Sotiropoulos, S. N. An integrated approach to correction for '
                        'off-resonance effects and subject movement in diffusion MR imaging. NeuroImage, 2015, 125, '
                        '1063-1078', True)
app.cmdline.addCitation('If ' + option_prefix + 'preprocessed is not used',
                        'Andersson, J. L. R. & Graham, M. S. & Zsoldos, E. & Sotiropoulos, S. N. Incorporating '
                        'outlier detection and replacement into a non-parametric framework for movement and '
                        'distortion correction of diffusion MR images. NeuroImage, 2016, 141, 556-572', True)

app.cmdline.addCitation('If using ' + option_prefix + 'parcellation aal, ' +
                        option_prefix + 'parcellation aal2, ' + option_prefix +
                        'parcellation craddock200, ' + option_prefix + 'parcellation craddock400 or ' +
                        option_prefix + 'parcellation perry512 and not using ' + option_prefix + 'template_reg fsl',
                        'Avants, B. B.; Epstein, C. L.; Grossman, M.; Gee, J. C. Symmetric diffeomorphic image registration with '
...
app.cmdline.addCitation('',
                        ...
                        'R. M. Co-registration and distortion correction of diffusion and anatomical images based on '
                        'inverse contrast normalization. NeuroImage, 2015, 115, 269-280', True)

app.cmdline.addCitation(
    'If using ' + option_prefix + 'parcellation craddock200 or ' + option_prefix + 'parcellation craddock400',
    'Craddock, R. C.; James, G. A.; Holtzheimer, P. E.; Hu, X. P.; Mayberg, H. S. A whole brain fMRI atlas generated '
    'via spatially constrained spectral clustering. Human Brain Mapping, 2012, 33(8), 1914-1928',
    True)

app.cmdline.addCitation(
    'If using ' + option_prefix + 'parcellation desikan, ' + option_prefix + 'parcellation destrieux or ' +
    option_prefix + 'parcellation hcpmmp1',
    'Dale, A. M.; Fischl, B. & Sereno, M. I. Cortical Surface-Based Analysis: I. Segmentation and Surface '
    'Reconstruction. NeuroImage, 1999, 9, 179-194',
    True)

app.cmdline.addCitation('If using ' + option_prefix + 'parcellation desikan',
                        'Desikan, R. S.; Segonne, F.; Fischl, B.; Quinn, B. T.; Dickerson, B. C.; Blacker, '
                        'D.; Buckner, R. L.; Dale, A. M.; Maguire, R. P.; Hyman, B. T.; Albert, M. S. & Killiany, '
                        'R. J. An automated labeling system for subdividing the human cerebral cortex on MRI scans '
                        'into gyral based regions of interest NeuroImage, 2006, 31, 968-980',
                        True)

app.cmdline.addCitation('If using ' + option_prefix + 'parcellation destrieux',
                        'Destrieux, C.; Fischl, B.; Dale, A. & Halgren, E. Automatic parcellation of human cortical '
                        'gyri and sulci using standard anatomical nomenclature NeuroImage, 2010, 53, 1-15',
                        True)

app.cmdline.addCitation('If using ' + option_prefix + 'parcellation hcpmmp1',
                        'Glasser, M. F.; Coalson, T. S.; Robinson, E. C.; Hacker, C. D.; Harwell, J.; Yacoub, '
                        'E.; Ugurbil, K.; Andersson, J.; Beckmann, C. F.; Jenkinson, M.; Smith, S. M. & Van Essen, '
                        'D. C. A multi-modal parcellation of human cerebral cortex. Nature, 2016, 536, 171-178',
                        True)

app.cmdline.addCitation(
    ('' if is_container else 'If using ROBEX for brain extraction'),
    'Iglesias, J. E.; Liu, C. Y.; Thompson, P. M. & Tu, Z. Robust Brain Extraction Across Datasets and Comparison '
    'With Publicly Available Methods. IEEE Transactions on Medical Imaging, 2011, 30, 1617-1634',
    True)

app.cmdline.addCitation('',
                        'Jeurissen, B; Tournier, J-D; Dhollander, T; Connelly, A & Sijbers, J. Multi-tissue '
                        'constrained spherical deconvolution for improved analysis of multi-shell diffusion MRI data '
                        'NeuroImage, 2014, 103, 411-426',
                        False)

app.cmdline.addCitation('If ' + option_prefix + 'preprocessed is not used',
                        'Kellner, E.; Dhital, B.; Kiselev, V. G.; Reisert, M. Gibbs-ringing artifact removal based on '
                        'local subvoxel-shifts. Magnetic Resonance in Medicine, 2006, 76(5), 1574-1581',
                        True)

app.cmdline.addCitation('',
                        'Patenaude, B.; Smith, S. M.; Kennedy, D. N. & Jenkinson, M. A Bayesian model of shape and '
                        'appearance for subcortical brain segmentation. NeuroImage, 2011, 56, 907-922',
                        True)

app.cmdline.addCitation('If using ' + option_prefix + 'parcellation perry512',
                        'Perry, A.; Wen, W.; Kochan, N. A.; Thalamuthu, A.; Sachdev, P. S.; Breakspear, '
                        'M. The independent influences of age and education on functional brain networks and '
                        'cognition in healthy older adults. Human Brain Mapping, 2017, 38(10), 5094-5114',
                        True)

if not is_container:
    app.cmdline.addCitation('If not using ROBEX for brain extraction',
                            'Smith, S. M. Fast robust automated brain extraction. Human Brain Mapping, 2002, 17, '
                            '143-155',
                            True)

app.cmdline.addCitation('',
                        'Smith, R. E.; Tournier, J.-D.; Calamante, F. & Connelly, A. Anatomically-constrained '
...
app.cmdline.addCitation('',
                        'Smith, R. E.; Tournier, J.-D.; Calamante, F. & Connelly, A. The effects of SIFT on the '
                        'reproducibility and biological accuracy of the structural connectome. NeuroImage, 2015a, '
                        '104, 253-265',
                        False)

app.cmdline.addCitation('',
                        'Smith, R. E.; Tournier, J.-D.; Calamante, F. & Connelly, A. SIFT2: Enabling dense '
                        'quantitative assessment of brain white matter connectivity using streamlines tractography. '
...
app.cmdline.addCitation('',
                        'Tournier, J.-D.; Calamante, F. & Connelly, A. Improved probabilistic streamlines '
                        'tractography by 2nd order integration over fibre orientation distributions. Proceedings of '
                        'the International Society for Magnetic Resonance in Medicine, 2010, 1670',
                        False)

app.cmdline.addCitation('If ' + option_prefix + 'preprocessed is not used',
                        'Tustison, N.; Avants, B.; Cook, P.; Zheng, Y.; Egan, A.; Yushkevich, P. & Gee, '
                        'J. N4ITK: Improved N3 Bias Correction. IEEE Transactions on Medical Imaging, 2010, 29, '
                        '1310-1320',
                        True)

app.cmdline.addCitation(
    'If using \'aal\' or \'aal2\' value for ' + option_prefix + 'parcellation option',
    'Tzourio-Mazoyer, N.; Landeau, B.; Papathanassiou, D.; Crivello, F.; Etard, O.; Delcroix, N.; Mazoyer, '
    'B. & Joliot, M. Automated Anatomical Labeling of activations in SPM using a Macroscopic Anatomical Parcellation '
...
app.cmdline.addCitation('',
                        'Yeh, C.H.; Smith, R.E.; Liang, X.; Calamante, F.; Connelly, A. Correction for diffusion MRI '
                        'fibre tracking biases: The consequences for structural connectomic metrics. Neuroimage, '
                        '2016, 142, 150-162',
                        False)

app.cmdline.addCitation('If using ' + option_prefix + 'parcellation perry512',
                        'Zalesky, A.; Fornito, A.; Harding, I. H.; Cocchi, L.; Yucel, M.; Pantelis, C. & Bullmore, '
                        'E. T. Whole-brain anatomical networks: Does the choice of nodes matter? NeuroImage, 2010, '
                        '50, 970-983',
                        True)
...
if app.args.analysis_level == 'participant':
    ...
    for subject_label in subjects_to_analyze:
        if app.args.longitudinal:
            sessions_to_analyze = []

            # Get sessions from call
            if app.args.session_label:
                sessions_to_analyze = ['ses-' + ses_index for ses_index in app.args.session_label]

                for session_dir in sessions_to_analyze:
                    if not os.path.isdir(os.path.join(app.args.bids_dir, subject_label, session_dir)):
                        app.error('Unable to find session directory: ' + session_dir)

            else:
                session_dirs = glob.glob(os.path.join(app.args.bids_dir, subject_label, 'ses-*'))
                sessions_to_analyze = ['ses-' + directory.split("-")[-1] for directory in session_dirs]

                if not sessions_to_analyze:
                    app.error('Could not find any sessions in subject directory')

            for session in sessions_to_analyze:
                app.console('Commencing execution for subject ' + subject_label + ' and session ' + session)
                runSubject(app.args.bids_dir, subject_label,
                           os.path.abspath(app.args.output_dir), session)
